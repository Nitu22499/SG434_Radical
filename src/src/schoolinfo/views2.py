...
def SchoolRTEDetailView(request,ac_year):
    ...
    for choice in choices:
        obj = SchoolRTE.objects.filter(class_name=choice[0], academic_year=ac_year, srte_school_id = request.user.school if request.user.is_school_admin else request.GET.get('school',1)).first()
        if obj:
            response.update({
                choice[0]: obj
            })
...
def SchoolEWSDetailView(request,ac_year):
    ...
    obj = SchoolEWS.objects.filter(class_name=choice, academic_year=ac_year, sews_school_id = request.user.school if request.user.is_school_admin else request.GET.get('school',1)).first()
    if obj:
        response.update({
            choice: obj
        })
...
def SchoolAnnualDetailView(request,ac_year):
    ...
    for choice in choices:
        obj = SchoolAnnual.objects.filter(class_name=choice[0],sa_school_name_id = request.user.school if request.user.is_school_admin else request.GET.get('school',1)).first()
        if obj:
            response.update({
                choice[0]: obj
            })
...
def SchoolIncentivesDetailView(request,ac_year):
    ...
    for choice in choices:
        obj = SchoolIncentives.objects.filter(class_name=choice[0],si_school_name = request.user.school if request.user.is_school_admin else request.GET.get('school',1)).first()
        if obj:
            response.update({
                choice[0]: obj
            })
