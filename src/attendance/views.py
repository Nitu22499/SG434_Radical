...
from .forms import StudentAttendanceCreateForm, EmployeeAttendanceCreateForm
...
class StudentAttendanceCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        attendance.student_attendance_school = self.request.user.school
        attendance.save()
        messages.success(self.request, 'Attendance record is saved and students are fetched')
        return redirect('attendance:student-update', attendance.id)
...
class EmployeeAttendanceCreateView(CreateView):
    ...
    def form_valid(self, form):
        attendance = form.save(commit=False)
        attendance.employee_attendance_school = self.request.user.school
        attendance.save()
        messages.success(self.request, 'Attendance record is saved and employees are fetched')
        return redirect('attendance:employee-update', attendance.id)
